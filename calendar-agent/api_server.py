...
def sync_calendars():
    ...
    try:
        ...
        # Automatically refresh burnout cache after syncing calendars
        try:
            print(f"DEBUG: Refreshing burnout cache after calendar sync", flush=True)
            batch_result_json = predict_burnout_batch_tool(
                user_id=user_id,
                sleep_time='00:00',  # Default values
                wake_time='08:00',    # Default values
                days_ahead=14
            )
            batch_result = json.loads(batch_result_json)
            if 'error' not in batch_result:
                print(f"DEBUG: Successfully cached burnout predictions for 14 days", flush=True)
            else:
                print(f"DEBUG: Failed to cache burnout predictions: {batch_result.get('error')}", flush=True)
        except Exception as e:
            # Don't fail the sync if cache refresh fails
            print(f"DEBUG: Error refreshing burnout cache (non-fatal): {e}", flush=True)
